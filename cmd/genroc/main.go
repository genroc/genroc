...
func main() {
	...
	logPayloads := flag.Bool("log-payloads", true, "Record the payload an audit event is about (process/task input and output, request/response bodies) on its log row. Stored whole: --log-payload-bytes moves a large value out of line, it does not cut it.")
	logPayloadBytes := flag.Int("log-payload-bytes", 2048, "Inline budget for one audit payload. Above it the largest values move into the object store, fetchable by the reference the row carries and retained with it; it is also the width of the console excerpt.")
