...
## Reading result the fetch result
...
Genroc validates the data against that schema, so invalid data ever gets in.

Result of the is then available as `self.result` in the `case` expression,
...
Shapes are a convenient way transform data.
