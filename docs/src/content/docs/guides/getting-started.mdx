...
- **`result_schema` types the outside world.** An HTTP response is untyped until you say
  what it is. Once declared, the response is validated before the instance moves on, and
  `self.result.ok` is readable and known to be a boolean.
