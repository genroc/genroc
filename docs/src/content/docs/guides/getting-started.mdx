...
That URL is the same schema `GET /process-schema.json` serves, republished here on every
change, so it works with no genroc running. A local server is still the right target when
you are on an unreleased build: point it at `http://localhost:8448/process-schema.json`.
