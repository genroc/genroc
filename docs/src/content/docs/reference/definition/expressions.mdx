...
| Operator | On | Notes |
| --- | --- | --- |
| `==` `!=` | anything | deep equality on objects and arrays |
| `<` `>` `<=` `>=` | numbers | both operands must be non-nullable |
| `+` | numbers, strings | string concatenation when either side is a string |
| `-` `*` | numbers | |
...
either `?? 0` or declaring the slot non-nullable.
