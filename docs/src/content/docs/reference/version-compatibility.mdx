...
order_pipeline  upgradable  compatible
invoice         breaking    breaking
...
**Two columns, because there are two questions** — and each is answered for every process,
so neither is left blank. A blank cell under a header reads as a question that went
unanswered; `nothing changed` and `new` therefore print in both.

| UPGRADE | |
|---|---|
| `upgradable` | every state an instance can be parked in fits the new definition |
| `breaking` | one does not; the detail names which |

| CONTRACT | |
|---|---|
| `compatible` | callers and parents still get the shape they were written against |
| `breaking` | they do not |
| `breaking (ignored)` | it broke, and `--ignore contract` excused it from the exit code |

Under the table, one block per process, and inside it **one row per addressable place that
was compared**, annotated with what became of it — `(ok)`, `(breaking upgrade)`,
`(breaking contract)`, or `(not judged)`. A place that holds up still gets a row, so you can
see what was examined rather than inferring it from silence. The findings sit under it: a
path *inside* that schema, and what differs.
...
`nothing changed` (both sides resolve to the same version, or a submitted document is
