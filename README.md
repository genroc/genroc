...
- **Bounded by default.** A fetch response is capped (a body past the limit raises the
  catchable `result.too_large` instead of taking the worker down with every lease it
...
`genctl apply`, `types` and `compat --from latest` need no file arguments. To narrow,
