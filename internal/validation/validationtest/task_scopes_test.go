...
var preOutputCases = map[string]struct {
	...
	"body":           {`{"id":"t","action":{"type":"fetch","method":"post","url":"http://x","body":{"v":"%s"}},` + loop, leaf, "%[1]s.n"},
	"url":            {`{"id":"t","action":{"type":"fetch","method":"post","url":"http://x/%s"},` + loop, interp, "%[1]s.n ?? 0"},
	"method":         {`{"id":"t","action":{"type":"fetch","url":"http://x","method":"%s"},` + loop, interp, "%[1]s.text ?? 'get'"},
	"headers":        {`{"id":"t","action":{"type":"fetch","method":"post","url":"http://x","headers":{"h":"%s"}},` + loop, interp, "%[1]s.n ?? 0"},
	"query":          {`{"id":"t","action":{"type":"fetch","method":"post","url":"http://x","query":{"q":"%s"}},` + loop, interp, "%[1]s.n ?? 0"},
	"acceptedStatus": {`{"id":"t","action":{"type":"fetch","method":"post","url":"http://x","accepted_status":["%s"]},` + loop, interp, "%[1]s.text ?? '2xx'"},
	...
	"on_error raise data": {`{"id":"t","action":{"type":"fetch","method":"post","url":"http://x"},
