...
func TestRenderEvent(t *testing.T) {
	...
	got := RenderEvent(TimeClock, ts, "info", "6fah8w2p", rec.Event, "fetch", rec.Detail(ModeDetail), true)
	want := "15:04:05  INFO   6fah8w2p  action_succeeded  fetch           status=200 result={\"slept\":5000}"
	if got != want {
		t.Errorf("RenderEvent(tree):\n got %q\nwant %q", got, want)
	}

	// basic mode drops the data body.
	got = RenderEvent(TimeClock, ts, "info", "6fah8w2p", rec.Event, "fetch", rec.Detail(ModeBasic), true)
	want = "15:04:05  INFO   6fah8w2p  action_succeeded  fetch           status=200"
	if got != want {
