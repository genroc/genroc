...
// Any other keyword causes an unmarshal error; an unrecognised type name is caught a
// step later, by CheckDoc (see the note there for why).
//
// The subset is strict in one direction only: everything genroc accepts is valid JSON
// Schema. It accepts less (no allOf, no boolean additionalProperties, no unrecognised
// keywords) and adds one inert annotation ("secret"), which JSON Schema permits since
// validators ignore keywords they do not know. Nothing here is rejected by a standard
// validator — a property worth preserving, since a genroc schema is published to
// external-task resolvers over the wire and generated by outside tooling. The empty
// node {} is therefore the top type as authored, not just as represented; see
// specs/unknown-type.md for why a dedicated spelling was tried and dropped.
//
