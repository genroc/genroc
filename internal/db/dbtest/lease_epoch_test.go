...
func TestLeaseEpoch_OperatorVerbsDoNotBump(t *testing.T) {
	for _, b := range testBackends(t) {
		t.Run(b.name, func(t *testing.T) {
			...
			if err := b.db.PauseProcess(ctx, "verbs-pause"); err != nil {
				t.Fatalf("PauseProcess: %v", err)
			}
			if err := b.db.ResumeProcess(ctx, "verbs-pause"); err != nil {
			...
			if err := b.db.RetryProcess(ctx, "verbs-retry", false); err != nil {
	...
	for _, b := range testBackends(t) {
		t.Run(b.name, func(t *testing.T) {
			...
			for i, sig := range []string{"s1", "s2"} {
				if delivered, err := b.db.DeliverSignal(ctx, "arm-1", "approval", sig, map[string]any{"n": i + 1}); err != nil || delivered {
